...
whoami_args="$(cat "$MOCK_CURL_ARGS_FILE")"
assert_contains "$whoami_args" "http://api.test/api/v1/me" "whoami path"
assert_not_contains "$whoami_args" "/api/v1/me/" "whoami bare path (no subpath)"
...
view_args="$(cat "$MOCK_CURL_ARGS_FILE")"
assert_contains "$view_args" "/docs/7" "view doc path"
assert_not_contains "$view_args" "/docs/7/versions" "view doc bare path (no version)"
